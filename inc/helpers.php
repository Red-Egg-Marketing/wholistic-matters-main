...
function get_attachment_fallback( $id = '', $size = 'medium_large', $attr = [] ) {
	...
	if ( ! $wp_img ) {
		$wp_img = '<img class="placeholder-img" src="' . IMAGE_PLACEHOLDER . '" width="139" height="70" alt=""';
...
//Convert embed format
function extractYouTubeVideoId($url) {
    $patterns = [
        '/(?:https?:\/\/)?(?:www\.)?youtu\.be\/([a-zA-Z0-9_-]{11})/',  // Shortened URL format
        '/(?:https?:\/\/)?(?:www\.)?youtube\.com\/watch\?v=([a-zA-Z0-9_-]{11})/'  // Full URL format
    ];
